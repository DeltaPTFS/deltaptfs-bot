function createDatabase(databaseUrl, PoolClass = null) {
  ...
  async function init() {
    await pool.query(`
      DO $$
      BEGIN
        IF to_regclass('public.authentications') IS NULL AND to_regclass('public.verifications') IS NOT NULL THEN
          ALTER TABLE verifications RENAME TO authentications;
        END IF;
        IF to_regclass('public.authentication_sessions') IS NULL AND to_regclass('public.verification_sessions') IS NOT NULL THEN
          ALTER TABLE verification_sessions RENAME TO authentication_sessions;
        END IF;
        IF to_regclass('public.guild_authentication_config') IS NULL AND to_regclass('public.guild_verification_config') IS NOT NULL THEN
          ALTER TABLE guild_verification_config RENAME TO guild_authentication_config;
        END IF;
      END $$;
      ...
      DO $$
      BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='authentications' AND column_name='verified_at') THEN
          ALTER TABLE authentications RENAME COLUMN verified_at TO authenticated_at;
        END IF;
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='guild_authentication_config' AND column_name='verified_role_id') THEN
          ALTER TABLE guild_authentication_config RENAME COLUMN verified_role_id TO authenticated_role_id;
        END IF;
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='guild_authentication_config' AND column_name='unverified_role_id') THEN
          ALTER TABLE guild_authentication_config RENAME COLUMN unverified_role_id TO unauthenticated_role_id;
        END IF;
      END $$;
      ...
      CREATE INDEX IF NOT EXISTS authentication_sessions_expiry_idx ON authentication_sessions(expires_at);
      CREATE TABLE IF NOT EXISTS guild_authentication_config (
        guild_id BIGINT PRIMARY KEY,
        authenticated_role_id BIGINT NOT NULL,
        unauthenticated_role_id BIGINT NOT NULL,
      ...
      CREATE TABLE IF NOT EXISTS guild_role_mappings (
        guild_id BIGINT NOT NULL REFERENCES guild_authentication_config(guild_id) ON DELETE CASCADE,
  ...
  async function createPending(session) {
    await pool.query('DELETE FROM authentication_sessions WHERE expires_at < NOW() OR discord_user_id = $1', [session.discordUserId]);
    await pool.query(`INSERT INTO authentication_sessions
  ...
  async function consumePending(stateHash) {
    const result = await pool.query(`DELETE FROM authentication_sessions
      WHERE state_hash = $1 AND expires_at > NOW()
      RETURNING discord_user_id, guild_id, expected_roblox_user_id, code_verifier, rp_name`, [stateHash]);
    return result.rows[0] ?? null;
  }

  async function saveAuthentication(record) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      const conflicts = await client.query(`SELECT discord_user_id, roblox_user_id FROM authentications
      ...
      if (conflicts.rows.some((row) => String(row.discord_user_id) !== String(record.discordUserId)
        ...
        error.code = 'DUPLICATE_AUTHENTICATION';
        throw error;
      }
      ...
      if (error.code === '23505') error.code = 'DUPLICATE_AUTHENTICATION';
      throw error;
  ...
  async function getGuildConfig(guildId) {
    const core = await one('SELECT * FROM guild_authentication_config WHERE guild_id = $1', [guildId]);
    if (!core) return null;
    ...
    return {
      ...
      authenticatedRoleId: String(core.authenticated_role_id),
      unauthenticatedRoleId: String(core.unauthenticated_role_id),
  ...
  async function saveGuildConfig(entry) {
    await pool.query(`INSERT INTO guild_authentication_config
      (guild_id, authenticated_role_id, unauthenticated_role_id, log_channel_id, roblox_group_id, updated_by, updated_at)
      VALUES ($1,$2,$3,$4,$5,$6,NOW())
      ON CONFLICT (guild_id) DO UPDATE SET authenticated_role_id=EXCLUDED.authenticated_role_id,
      unauthenticated_role_id=EXCLUDED.unauthenticated_role_id, log_channel_id=EXCLUDED.log_channel_id,
      roblox_group_id=EXCLUDED.roblox_group_id, updated_by=EXCLUDED.updated_by, updated_at=NOW()`,
    [entry.guildId, entry.authenticatedRoleId, entry.unauthenticatedRoleId, entry.logChannelId, entry.robloxGroupId, entry.updatedBy]);
